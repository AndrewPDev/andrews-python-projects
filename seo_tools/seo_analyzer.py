...
class SEOAnalyzer:
    ...
    def AnalyzeWebsite( self, url: str ) -> Dict:
        print( f"\nStarting SEO analysis for: {url}" )
        print( "=" * 60 )
        
        try:
            html_content, status_code = self._FetchWebpage( url )
            if not html_content:
                return {'error': f'Could not fetch webpage. Status code: {status_code}'}
            
            parser = SEOHTMLParser()
            parser.base_url = self._ExtractBaseUrl( url )
            parser.feed( html_content )
            
            self.results = {
                'url': url,
                'title_analysis': self._AnalyzeTitle( parser.title ),
                'meta_analysis': self._AnalyzeMetaDescription( parser.meta_description ),
                ...
                'technical_analysis': self._AnalyzeTechnical( url, html_content ),
                'meta_tags': parser.meta_tags
            }
            
            self._CalculateSeoScore()
    ...
    def _FetchWebpage( self, url: str ) -> Tuple[Optional[str], int]:
        try:
            headers = {
    ...
    def _ExtractBaseUrl( self, url: str ) -> str:
    ...
    def _AnalyzeMetaDescription( self, description: str ) -> Dict:
        analysis = {
    ...
    def _AnalyzeTechnical( self, url: str, html_content: str ) -> Dict:
        analysis = {
    ...
    def PrintDetailedReport( self ):
        ...
        print( f"Overall SEO Score: {self.score}/10 {self._GetScoreRating( self.score )}" )
        print( f"{'='*60}" )
    ...
    def _GetScoreRating( self, score: int ) -> str:
        if score >= 8:
            return "Excellent"
        elif score >= 6:
            return "Good"
    ...
    def _GenerateRecommendations( self ):
        self.recommendations = []
    ...
    def ExportReport( self, filename: str ):
        try:
        ...
        try:
            ...
            if user_input.lower() in ['quit', 'exit', 'q']:
                ...
                if last_analysis:
                    ...
                    analyzer.ExportReport( filename )
                else:
            ...
            results = analyzer.AnalyzeWebsite( user_input )
