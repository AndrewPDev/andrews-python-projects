...
class HashIdentifier:
    ...
    def IdentifyHash( self, hash_string: str ) -> List[Dict[str, str]]:
        hash_string = hash_string.strip()
        matches = []
        
        for hash_type, properties in self.hash_patterns.items():
            if self._MatchesPattern( hash_string, properties ):
                confidence = self._CalculateConfidence( hash_string, properties )
                matches.append( {
                    'type': hash_type,
    ...
    def _MatchesPattern( self, hash_string: str, properties: Dict ) -> bool:
    ...
    def _CalculateConfidence( self, hash_string: str, properties: Dict ) -> int:
        confidence = 0
    ...
    def AnalyzeHashFile( self, filename: str ) -> Dict[str, List[Dict]]:
        results = {}
        try:
            with open( filename, 'r', encoding='utf-8' ) as file:
                for line_num, line in enumerate( file, 1 ):
                    ...
                    if line and not line.startswith( '#' ):  # Skip empty lines and comments
                        matches = self.IdentifyHash( line )
    ...
    def PrintResults( self, matches: List[Dict[str, str]], hash_string: str ):
        print( f"\n{'='*60}" )
        ...
        # Additional analysis
        self._PrintAdditionalAnalysis( hash_string, matches )
    
    # Print additional analysis information
    def _PrintAdditionalAnalysis( self, hash_string: str, matches: List[Dict] ):
    ...
    while True:
        try:
            ...
            if user_input.lower() in ['quit', 'exit', 'q']:
                ...
                results = identifier.AnalyzeHashFile( filename )
            ...
            matches = identifier.IdentifyHash( user_input )
            identifier.PrintResults( matches, user_input )
